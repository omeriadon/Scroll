...
final class MacHostManager {
	static let shared = MacHostManager()
	...
	var approvedDevices: [DeviceInfo] {
		Defaults[.approvedDevices]
	}
	...
	private func setupNetworkListener() {
		...
		networkListener.onCommandReceived = { [weak self] command in
			...
			if isScrollingEnabled, isAccessibilityTrusted {
				scrollExecutor.executeScrollCommand(command)
			}
	...
	private func refreshConnectionStatus() {
		...
		let pairedCount = clients.values.filter(\.isPaired).count
