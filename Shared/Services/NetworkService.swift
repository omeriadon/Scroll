...
// MARK: - Mac UDP Listener
	...
	public final class MacScrollNetworkListener {
				...
				switch state {
					...
					case .failed, .cancelled:
						Task { @MainActor [weak self] in
							...
							for (key, client) in connectedClients where client.connectionID == id {
							...
							_connectionsForCleanup.removeAll { $0 === conn }
							onStateChanged?()
						}
					default: break
				}
		...
		private nonisolated func startReceiving(from conn: NWConnection, id: String) {
			conn.receiveMessage { [weak self] data, _, _, error in
				if let error {
					if case let .posix(code) = error, code == .ECANCELED { return }
				...
				if let data, !data.isEmpty {
					...
					if PairingMessage.isPairingMessage(data) {
						...
						Task { @MainActor [weak self] in
							...
							let isPaired = connectedClients.values.contains {
								$0.connectionID == id && $0.isPaired
							}
							if isPaired {
								// Update last seen timestamp for scroll activity
								for (_, client) in connectedClients where client.connectionID == id {
			...
			if approvedDevices.contains(where: { $0.id == deviceInfo.id }) {
				...
				onPairingRequest?(deviceInfo) { [weak self] approved in
					Task { @MainActor [weak self] in
						...
						sendPairingResponse(to: connectionID, approved: approved)
						onStateChanged?()
					}
				}
			}
...
// MARK: - iPhone UDP Client
	...
	public struct DiscoveredMac: Sendable, Hashable, Identifiable {
		...
		public var id: String {
			if case let .service(name, _, _, _) = browseResult.endpoint {
				return name
			}
	...
	public final class iPhoneScrollNetworkClient {
		...
		public func startDiscovery() {
			...
			b.browseResultsChangedHandler = { [weak self] results, _ in
				Task { @MainActor [weak self] in
					...
					discoveredHosts = results.map { result in
		...
		private func attemptAutoConnect() {
			...
			if let mac = discoveredHosts.first(where: { $0.deviceInfo?.id == lastMac.id }) {
				...
				if case let .service(name, _, _, _) = host.browseResult.endpoint {
		...
		private nonisolated func startReceiving(from conn: NWConnection) {
			conn.receiveMessage { [weak self] data, _, _, error in
				if let error {
					if case let .posix(code) = error, code == .ECANCELED { return }
					return
