...
public extension Defaults {
	...
	enum iCloud {
		...
		/**
		 The synced keys.
		 */
		public static var keys: Set<Defaults.Keys> {
			synchronizer.keys
		}
...
final class iCloudSynchronizer {
	 ...
	 */
	func add(_ key: Defaults.Key<some Defaults.Serializable>) {
		let (isInserted, _) = keys.insert(key)
...
extension iCloudSynchronizer {
	...
	private func willEnterForeground(notification _: Notification) {
		remoteStorage.synchronize()
	}
